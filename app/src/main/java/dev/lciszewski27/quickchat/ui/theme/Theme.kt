...
// ── Expressive presets (same structure as reference app) ─────────────
private val ChatTealLight = lightColorScheme(
...
private val ChatTealDark = darkColorScheme(
...
fun QuickChatTheme(
    ...
    var colorScheme = when {
        dynamicColor -> {
            ...
            when (colorPreset) {
                ColorPreset.CHAT_TEAL -> if (darkTheme) ChatTealDark else ChatTealLight
    ...
    val currentFontFamily: FontFamily = if (useSystemFont) {
        FontFamily.Default
    } else {
        QuicksandFontFamily
    }
